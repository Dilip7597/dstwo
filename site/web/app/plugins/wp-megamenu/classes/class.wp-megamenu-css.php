...
if ( ! class_exists('wp_megamenu_css')) {
    ...
    class wp_megamenu_css{
        ...
        public function css(){
            ...
            //die(print_row($locations));

            $style = "";
            if (empty($auto_intergration_menu)){
                return;
            }
            foreach ($auto_intergration_menu as $nav_key => $nav_value) {
                if (!empty($locations[$nav_value])) {

                    $nav_id = (int)$locations[$nav_value];
