    ...
    class wp_megamenu extends Walker_Nav_Menu{
        ...
        function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
            ...
            elseif ( $item->type === 'widget'){
                ...
                if ( (is_array($wpmm_item_settings) &&  empty($wpmm_item_settings['options']['hide_arrow']) ) || (is_array($wpmm_item_settings) && ! empty($wpmm_item_settings['options']['hide_arrow']) && $wpmm_item_settings['options']['hide_arrow'] !=
                'true'  ) ) {
...
function overrite_functions_wp_megamenu($args){
    ...
    if (count($auto_intergration_menu)){
        ...
        return $argunets = array(
            ...
            'container_class'   => "wp-megamenu-wrap {$sticky_class} {$menu_layout_class} {$menu_custom_class} ",
